class Visualizer:
    ...
    def _color(text, color_name):
        ...
        """
        return (
            f"{Visualizer.COLORS.get(color_name, '')}{text}{Visualizer.COLORS['reset']}"
        )
    ...
    def render_gantt(res: dict, custom_colors: dict = None) -> None:
        ...
        print("".join(axis))
        labels = ["     "] + [
            f"{t:<5}" if t % 5 == 0 else "" for t in range(total_time)
        ]
    ...
    def display_summary(res: dict) -> None:
        ...
        for r in sorted(res["individual_results"], key=lambda x: x["pid"]):
            print(
                f"P{r['pid']:02}  | {r['arrival']:<8} | {r['burst']:<6} | {r['wait']:<6.1f} | {r['turnaround']:<6.1f} | {r['completion']:<8}"
            )

        print("-" * len(head))
        avgs = res["averages"]
        print(
            f"Average Waiting Time:    {Visualizer._color(avgs['avg_waiting_time'], 'green')}"
        )
        print(
        ...
        print(
            f"CPU Utilization:         {Visualizer._color(avgs['cpu_utilization'], 'cyan')} (Non-Idle Time / Total Time)"
        )
    ...
    def display_audit(res: dict) -> None:
        ...
        for r in sorted(res["individual_results"], key=lambda x: x["pid"]):
            ...
            print(
                f"  └─ Turnaround: {r['completion']} (End) - {r['arrival']} (Start) = {Visualizer._color(r['turnaround'], 'white')}"
            )
            print(
                f"  └─ Wait:       {r['turnaround']} (TAT) - {r['burst']} (Burst) = {Visualizer._color(r['wait'], 'white')}"
            )
